...
const prInfoMap: Map<number, {commentCount: number, linesChanged: number, hoursToMerge: number, mergedAt: Date}> = new Map()
...
for await (const page of octokit.graphql.paginate.iterator(prQuery,{cursor: null})) {
        ...
        for (const pr of page.repository.pullRequests.nodes) {
            ...
            const reviewComments = pr.reviewThreads.nodes.reduce((sum:number, node:any) => sum + node.comments.totalCount, 0);
